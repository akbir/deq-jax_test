...
class TestBroyden:
    def test_optima(self):
        ...
        threshold = 30
        eps = 1e-6
        th_input = torch.tensor(values, dtype=torch.float32)
        jax_input = jnp.asarray(values, dtype=jnp.float32)

        broyden_jit = jax.jit(broyden, static_argnums=(0, 2, 3))

        jax_ans = broyden_jit(jax.jit(quadratic), jax_input, threshold, eps)
        th_ans = th_broyden(quadratic, th_input, threshold, eps)
        ...
        np.testing.assert_array_almost_equal(np.array(jax_ans['diff_detail']), th_ans['diff_detail'].numpy(), decimal=5)
